...
- Poll up to 20 times (sleep 15 seconds before each check, 5 minutes total):
  - Each cycle starts with `sleep 15`, then checks both APIs for each bot using `run_command`:
...
- Read ALL review comments and formal reviews carefully — from both Claude and Gemini (and any human reviewers) using `view_file` or `run_command` (e.g. `gh pr view`).
