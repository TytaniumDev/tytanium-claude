---
name: overnight
description: Run a long, autonomous code-improvement session. Multiple critic sub-agents scan the repo in parallel, propose fixes, and ship them as themed PRs. Bounded by a 5-hour wall clock and a 20-PR cap. Self-merges when CI passes. Use this when the user wants to run an autonomous overnight or long-running codebase improvement session.
---
...
Files to create (use the `write_to_file` tool):
...
Read every file in `agent-memory/` using the `view_file` tool, and **load into your active context**:
...
Use the **`invoke_subagent` tool** to launch the applicable critics simultaneously (specifying the subagents in the `Subagents` array of a single tool call). You can use the `research` subagent type or define custom critic subagents. Each critic gets the prompt template below filled in with its specialty. Each critic must return a structured list of proposals.
...
INSTRUCTIONS:
1. Scan the codebase for issues within your domain. Use view_file, grep_search, list_dir.
...
Launch these in **parallel** (using a single `invoke_subagent` call with multiple entries in `Subagents` array). Each returns a JSON array of proposals.
...
Use the `invoke_subagent` tool with the `self` or a custom subagent type with this prompt:
...
2. Implement the proposals. Use view_file, replace_file_content, and write_to_file.
...
8. Open the PR targeting $WORKING_BRANCH (NOT main):
   ...
   - "Generated by Antigravity /overnight run <RUN_ID>"
...
Once CI is green, dispatch the **reviewer** subagent using `invoke_subagent` (with `self` or a custom subagent type):
...
Once the loop exits (deadline hit, PR cap hit, or backlog empty), dispatch the **historian** subagent using `invoke_subagent` (with `self` or a custom subagent type):
